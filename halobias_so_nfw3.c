...
/*---Read-Arguments-------------------------------------------------------------*/
  ...
  sscanf(argv[1],"%lf",&logMmin) ;
  Mmin=pow(10.,logMmin) ;
  sscanf(argv[2],"%lf",&siglogM) ;
  sscanf(argv[3],"%lf",&logM0) ;
  M0=pow(10.,logM0) ;
  sscanf(argv[4],"%lf",&logM1) ;
  M1=pow(10.,logM1) ;
  sscanf(argv[5],"%lf",&alpha) ;
  sscanf(argv[6],"%d",&center) ;
  sscanf(argv[7],"%lf",&gamma) ;
  sscanf(argv[8],"%lf",&fgal) ;
  sscanf(argv[9],"%lf",&M_STAR) ;
  sscanf(argv[10],"%lu",&seed) ; // This seed is for choosing number of galaxies
...
/*---Read-group-info-in-BGC2-files-----------------------------------------------------*/
  ...
  Parent=(int64_t *) calloc(Nhalos,sizeof(int64_t)) ;
...
/*---Loop-over-halos------------------------------------------------------------*/
  for(i=0;i<Nhalos;i++)
    {
      if(Parent[i] == (int64_t)-1 && Mhalo[i]>=Mmin) /* just using the host halos */
	{
	  //fprintf(stdout,"%d %e %f\n",Parent[i],Mhalo[i],Rvir[i]) ;
...
/*---Write-galaxy-file----------------------------------------------------------*/
  ...
  if(Ngal < 1000.)
    {
      fprintf(stderr,"halobias> Too few galaxies!i\n");
      return -1; 
    }
